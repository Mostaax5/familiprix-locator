...
class PerformanceContractTests(unittest.TestCase):
    ...
    def test_product_media_is_cached_and_loaded_without_request_flooding(self):
        ...
        self.assertIn('_PRODUCTS_PAYLOAD_VERSION = "disk-gzip-v1"', product_routes)
        etag_start = product_routes.index("etag = hashlib.sha256")
        self.assertIn("target_key", product_routes[etag_start:etag_start + 300])
        self.assertIn("@_serialized_product_corpus", product_routes)
        self.assertIn("bootstrap_product_payload(item)", product_routes)
        self.assertIn("_PRODUCT_PAYLOAD_CHUNK_BYTES = 64 * 1024", product_routes)
        self.assertIn("gzip.GzipFile(", product_routes)
        self.assertIn("os.replace(gzip_temp, gzip_path)", product_routes)
        self.assertIn("warm_product_payload_cache(blocking=False)", product_routes)
        self.assertIn("response = send_file(", product_routes)
        app_source = (ROOT / "app.py").read_text(encoding="utf-8")
        self.assertIn("payload = warm_product_payload_cache()", app_source)
